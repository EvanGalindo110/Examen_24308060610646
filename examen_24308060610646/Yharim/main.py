...
def editar():
    ...
    usuario_actual = usuarios.find_one({"correo": session['usuario']})
    ...
    if request.method == 'POST':
        ...
        if not nombre or not apellidos or not correo:
            ...
            return render_template('editar.html', datos=usuario_actual, error=error)
    ...
    return render_template('editar.html', datos=usuario_actual)
...
def listas():
    ...
    try:
        return render_template('listas.html', usuario=session['usuario'])
    except Exception as e:
        return f"Error al cargar la página de listas: {e}"
